...
<body>
    ...
    <div class="container">
        ...
        <div class="table-responsive">
            <form action="{{ URL::to('/paymentonl') }}" id="create_form" method="post">
                @csrf
                @foreach ($ctkh_onl as $key => $values)
                    <div class="form-group">
                        <label for="language">Khóa học </label>
                        <input name="order_type" id="order_type" class="form-control" value="{{ $values->tenkh_onl }}"
                        readonly />
                    </div>
                    <div class="form-group">
                        <label for="language">Tên khách hàng</label>
                        <input name="order_user" id="order_user" class="form-control" value="{{ $hocvien->tenhv }}"
                             />
                    </div>
                    {{-- <div class="form-group">
                    ...
                    <input class="btn btn-primary" action="action" onclick="window.history.go(-1); return false;" type="submit"
                        value="Quay trở lại" />
                @endforeach
            </form>
        </div>
